...
ctrl.dx = @(q,p,u,t) [zeros(2) eye(2); -eye(2) -sys.D(q)]*[sys.Ddq(q,p); sys.Ddp(q,p)] + [zeros(2); sys.G(q)]*u+ [zeros(2); sys.G(q)]*0*[sin(t);cos(t)];
...
K = tril(ones(2));
k2=diag([1;1]);
phi = @(q,p) k2*(atan(K*q)+p);%Here q is a variable to be determined
sim.z = -phi(ctrl.eq(0,sim.q0),ctrl.ep(0,sim.q0,ctrl.p(sim.q0,sim.p0)));
sim.x0 = [sim.q0; sim.p0;sim.z];
...
% Kd=[2,0;0,2];
% he = 0.5*dphideq(t_sym,q_sym)*(Kd)*dphidp()';%这里的系数对系统收敛到滑模面上有影响
...
epslion=-0;
ctrl.v = @(t,q,p) epslion*(ctrl.ep(t,q,p)+dphidp(p)'*dUdPhi(t,q,p))+(-inv(dphidp())*Lambda(t,q)*dUdPhi(t,q,p)+ctrl.D(q)*ctrl.ep(t,q,p)-((inv(dphidp())*dphideq(t,ctrl.eq(t,q)))*ctrl.T(q))*ctrl.ep(t,q,p));
...
% ctrl.u1 = @(t,q,p,z)  Lambda(t,q,p)* (phi(ctrl.eq(t,q),ctrl.ep(t,q,p))+z)/sqrt(sum(phi(ctrl.eq(t,q),ctrl.ep(t,q,p))+z).^2);
 % ctrl.u1 = @(t,q,p,z) 1*Lambda(t,ctrl.eq(t,q),ctrl.ep(t,q,p))* (sign(10*(phi(ctrl.eq(t,q),ctrl.ep(t,q,p))+z)));
 ctrl.u1 = @(t,q,p,z) 1*sign(phi(ctrl.eq(t,q),ctrl.ep(t,q,p)).'+z.')';

 %ctrl.u1 = @(t,q,p,z) 0;
%input u from eq.(20)
ctrl.u = @(t,q,p,z) ctrl.G(q)\(ctrl.D(q)*ctrl.pd(t,q) + ctrl.dpddq(t,q)*(ctrl.T(q)*p) + ctrl.T(q)\ctrl.ddqd(t) + ctrl.T(q)'*sys.dVdq(q) + ctrl.v(t,q,p)-ctrl.u1(t,q,p,z));

% Define  closed-loop energy in eq.(24)
ctrl.KE = @(t,q,p) 0.5*sum(ctrl.ep(t,q,p).^2);%kinetic energy
ctrl.U=matlabFunction((sum(normPhi.^2)),'vars',[{t_sym},{q_sym},{p_sym}]);%potential energy
ctrl.V =@(t,q,p,z) sqrt(sum(abs(phi(ctrl.eq(t,q),ctrl.ep(t,q,p))+z)));
ctrl.Hsmc = @(t,q,p,z) ctrl.KE(t,q,p) + ctrl.U(t,q,p) +ctrl.V(t,q,p,z);
%% Run simulation


% Comcatinate model with control law
%把ode和z_dot放进ode迭代的函数odeFun里
%ode = @(t,x) dx(x(1:2),x(3:4),ctrl.u(t,x(1:2),ctrl.p(x(1:2),x(3:4))),t);
ctrl.z_dot =@(t,q,p) 1*Lambda(t,ctrl.eq(t,q),ctrl.ep(t,q,ctrl.p(q,p)))*dUdPhi(t,q,p);
% Solve ODE
options = odeset('OutputFcn', @myOutputFcn,'RelTol',1e-1);
tic
[res.t,res.x] = ode45(@(t, x) odeFun(t, x, ctrl),[0:sim.delta_t:sim.t_end],sim.x0,options);
toc
%% Plot output
...
for i=1:length(res.t)
    ...
    res.Hsmc(i) = ctrl.Hsmc(res.t(i),res.q(i,:).',res.p(i,:).',res.x(i,5:6).');
    res.qd(i,:) = ctrl.qd(res.t(i)).';
    res.pd(i,:) =ctrl.pd(res.t(i),ctrl.qd(res.t(i)))';
    res.qe(i,:) = res.q(i,:) - res.qd(i,:);
    res.pe(i,:) = res.p(i,:) - res.pd(i,:);
     res.phi(i,:)=phi(res.qe(i,:).',res.pe(i,:).')+res.x(i,5:6).';
     % res.phi(i,:)=phi(res.qe(i,:).',res.pe(i,:).');
    res.u(i,:)  =ctrl.u(res.t(i,:).',res.q(i,:).',ctrl.p(res.q(i,:).',res.p0(i,:).'),res.x(5:6).');
     % res.u1(i,:) = ctrl.u1(res.t(i,:).',res.q(i,:).',res.p(i,:).',res.x(5:6).');
     res.u1(i,:) = 10*sign(res.phi(i).'+res.x(i,5:6).')';
end
...
function dxdt = odeFun(t, x ,ctrl)

    % Extract system states
    q = x(1:2);  % Generalized coordinates
    p = x(3:4); % Generalized momenta
    z = x(5:6);  % Sliding mode control variable
    % Compute dx (system dynamics) for q and p
    dx_qp = ctrl.dx(q, p, ctrl.u(t, q, ctrl.p(q, p),z), t);

    % Compute dz (sliding mode control) for z
    dz = ctrl.z_dot(t, q,p);  % z_dot depends on x, so we pass the full state vector

    % Concatenate results into dxdt
    dxdt = [dx_qp; dz];
end
