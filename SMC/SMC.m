...
a=6*sat(7,6)
function result = sat(lambda_hat, lambda_max)
    if lambda_hat >= lambda_max
        result = 1;
    else
        result = lambda_hat / lambda_max;
    end
end
% % 系统参数
% m1 = 1; m2 = 1; % 关节质量
...
% 
%     % 动力学模型 (简化欧拉积分)
%     theta_ddot = M \ (tau - C*theta_dot - G+30);
% 
%     % 状态更新
...
%     % 计算趋近律 s_dot
%     K2 = diag([60,60]);
%     s_dot = -K * abs(s).*sign(s)-K2*s; 
%     % 计算控制律 tau
%     tau = M * (theta_ddot_d - lambda * e_dot + s_dot) + C *theta_dot + G;%LSM
