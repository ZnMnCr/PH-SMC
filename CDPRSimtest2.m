...
% Define initial conditions
    sim.q0 = [-3 3 3 3 -3 0].';%Define the inital value of configuration
...
%    options = odeset('OutputFcn', @myOutputFcn,'RelTol',1e-1,'Events', @(t, x) saturationEvent(x, kMax));%Set options for ode solver
...
% ieout = [];
tic
  [res.t,res.x] = ode78(@(t,x) plant(t,x,sys,ctrl),sim.tstart:sim.delta_t:sim.t_end ,sim.x0,options);
%  while 1
...
%  if res.t(end)>=sim.t_end - sim.delta_t
%      break
...
%     xe(end,67) = antiSat(xe(end,:), kMax);
%     xe(end,67)
%     sim.x0 1`= xe(end,:);
%     sim.tstart = te(end);
%    if sim.tstart >= sim.t_end - sim.delta_t
%        break;
%    end
...
res.k4 = kMax*((res.x(:,67) >= kMax) + (res.x(:,67) < kMax).* (res.x(:,67) / kMax)); 
