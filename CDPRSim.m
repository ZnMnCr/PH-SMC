...
%定义轨迹
ctrl.qd = @(t) [10*(1/2)*cos(t)+5*(1/2)*sin(2*t); 10*(1/2)*cos(t);0;0;0;0];
ctrl.dqd = @(t) [-10*(1/2)*sin(t)+10*(1/2)*cos(2*t); -10*(1/2)*sin(t);0;0;0;0];
ctrl.ddqd = @(t) [-10*(1/2)*cos(t)-20*(1/2)*sin(2*t); -10*(1/2)*cos(t);0;0;0;0];
...
%VI. NUMERICAL EXAMPLE Case1 and K
 K=tril(ones(6));
K1=diag([100;100;100;100;100;100]);

 %phi=@(q,p) (K*q+p);%Here q is a variable to be determined
phi=@(q,p) K1*atan(q)+p;
...
he=600000*0.5*(dphideq(t_sym,q_sym)*ctrl.T(q_sym))*dphidp()';%这里的系数对系统收敛到滑模面上有影响
...
ctrl.v=@(t,q,p) (-Lambda(t,q)*dUdPhi(t,q,p)+(ctrl.D(q))*ctrl.ep(t,q,p)-((inv(dphidp())*dphideq(t,q))*ctrl.T(q))*ctrl.ep(t,q,p));
